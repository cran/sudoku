sudoku <- function(z, verbose=FALSE, map=c(1:9,letters), level=0, dir=".") {
  if (length(z)==1) {                    # if "z" is a filename, parse the file
    z <- scan(file.path(dir, z), "", quiet=TRUE)
    z <- do.call(rbind, strsplit(z, ""))
    z <- matrix(match(z, map, 0), nrow(z))
  }
  ...
  repeat {
    ...
    if (ngot==N^2) {print(matrix(map[z],N),,FALSE,,,TRUE);return(invisible(z))}
    if (ngot==oldngot) {                               # Failed.  Take a guess!
      ...
      zg <- z
      zg[ij[1], ij[2]] <- k
      res <- Recall(zg, verbose, map, level+1)
